...
return full.map(item => ({
  frame: item.frame,
  outOfBoundsCount: item.outOfBounds.length,
  blockOverlapCount: item.blockOverlaps.length,
  textOverlapCount: item.textOverlaps.length,
  sampleBlockOverlaps: item.blockOverlaps.slice(0, 5),
  sampleTextOverlaps: item.textOverlaps.slice(0, 8)
}));
...
function isLayoutBlock(node) {
  ...
  if (name.includes("detail panel")) return false;
...
function summary(node, box) {
  return { name: node.name, type: node.type, box };
}
