...
const frameId = "45:11759";
const outDir = join(process.cwd(), "tmp", "figma-unity-elementwise");
const assetDir = join(process.cwd(), "Assets", "AshfallCamp", "Art", "UI", "FigmaExports", "CampDashboardElementwise", "ElementAssets");
...
function renderMode(node, children) {
  ...
  if (node.type === "TEXT") {
    return textLooksLikeIcon(node) ? "visual" : "text";
  }
  if (children.length === 0) {
    return hasRenderablePaint(node) ? "visual" : "container";
  }
...
async function exportNode(item, index) {
  ...
  let target = node;
  let cleanup = null;
  if (item.mode === "text" && node.type === "TEXT") {
    const frame = figma.createFrame();
    frame.name = "__codex_text_export_frame__";
    frame.fills = [];
    frame.strokes = [];
    frame.clipsContent = false;
    frame.resize(Math.max(1, item.bounds.width), Math.max(1, item.bounds.height));
    ...
    target = frame;
    cleanup = frame;
  } else if (item.mode === "background" && "children" in node) {
    const clone = node.clone();
...
for (let i = 0; i < exports.length; i++) {
  exported.push(await exportNode(exports[i], i));
}
...
try {
  await send("initialize", {
    ...
    capabilities: {},
    clientInfo: { name: "codex-ashfall-elementwise-export", version: "1.0.0" },
  });
  ...
  for (const item of payload.exports || []) {
    ...
    const unityPath = `Assets/AshfallCamp/Art/UI/FigmaExports/CampDashboardElementwise/ElementAssets/${fileName}`;
  ...
  const jsonPath = join(outDir, "camp-dashboard-elementwise-tree.json");
