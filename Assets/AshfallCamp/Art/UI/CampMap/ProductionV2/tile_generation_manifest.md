...
Sheets 11 and 12 add twelve future expedition locations ordered from threat 1 (`ruined_homestead`) to threat 12 (`warlord_fortress`). They are included in the palette and preview catalog for manual map authoring.

Sheets 13 and 14 add six vivid grass-covered terrain tiles and six winter terrain tiles. They are available in the palette and brush without modifying scene Tilemaps.
...
`Markers/location_markers_sheet_v6.png` is the authoritative 6x5 marker sheet. The builder extracts its first 29 pins into transparent 256x256 Single Sprites, creates collider-free marker Tile assets, a separate marker palette and a marker preview catalog. Marker placement on scene Tilemaps is manual.
...
The editor-only `AshfallCampProductionV2Brush.asset` appears in the Tile Palette brush selector. Its inspector exposes all 97 tiles in manifest order and paints one selected tile per cell; the builder also activates the brush and the Production V2 palette after rebuilding. Both content and marker palette Grids use 5x5 world-unit cells, matching map cells, so thumbnails and paint positions cannot overlap.

`tile_radioactive_sea_border.png` is a standalone map-border tile: an impassable luminous radioactive sea for manually enclosing the authored map. It is imported and catalogued with the sheet-generated tiles but is not regenerated from a 3x2 sheet.
...
The Production V2 builder is asset-only. It never opens, clears, generates or saves `WorldTileMap`; the authored map remains entirely under manual control.
